...
def mk_cost_lines(x, y, w, b, ax):
    for i in range(len(x)):
        f_wb = w * x[i] + b  # Compute the predicted value
        ax.plot([x[i], x[i]], [y[i], f_wb], color=dldarkred, linestyle="dotted")
...
def plt_intuition(x_train, y_train):
    ...
    @interact(w=(w_range[0], w_range[1], 5, continuous_update=False))
    def func(w=150):
        f_wb = np.dot(x_train, w) + tmp_b
        ...
        ax[1].plot(w_array, cost)
        cur_cost = compute_cost(x_train, y_train, w, tmp_b)
        ax[1].scatter(w, cur_cost, s=100, color=dldarkred, zorder=10)

        ax[0].set_title("Housing Prices vs. Size")
        ax[1].set_title("Cost vs. Weight (w)")
        ax[0].set_xlabel("Size (1000 sqft)")
        ax[0].set_ylabel("Price (1000s of dollars)")
        ax[1].set_xlabel("Weight (w)")
        ax[1].set_ylabel("Cost")

        # Add dynamic cost text
        ax[1].text(0.5, 0.9, f'Current Cost: {cur_cost:.2f}', ha='center', va='center', transform=ax[1].transAxes, fontsize=12, bbox=dict(facecolor='white', alpha=0.5))
        plt.show()
"""        
...
def plt_contour_wgrad(x, y, hist, ax, w_range=[-100, 500, 5], b_range=[-500, 500, 5],
    ...
    for point in hist[0::step]:
        #Euclidean distance between the current point and the base point.
